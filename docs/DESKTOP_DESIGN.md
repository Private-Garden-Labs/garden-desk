...
Garden Desk V1 is a calm, conversation-centered desktop agent inspired by the structural clarity of the Codex app without copying its branding or visual assets. The interface exposes work and context, not model infrastructure.
...
│                      ├─────────────────────────────────────────────────────┤
│                      │ context chips                                       │
│ Settings             │ Ask Garden Desk…                         Stop/Send    │
└──────────────────────┴─────────────────────────────────────────────────────┘
```
...
On macOS the sidebar background extends beneath the native traffic-light controls, which use equal top and left insets and align vertically with the conversation name, and the native title text is hidden. The header remains draggable around its controls. The composer stays anchored to the bottom of the workspace. On Windows the system draws the title bar above the application, so the sidebar keeps no space for window controls. A lightweight conversation header shows the conversation name with a rename control, one model control that holds the on-device state and the manual unload action, a compact appearance control, and Technical details without displacing folder navigation. The rename control opens a small panel that accepts a name of 60 characters or less. The approved model name stays in the tooltip of the model state. Runtime memory budget, measured RAM and VRAM or unified-memory allocation, allocated context, hardware cap, and its reason appear only in Technical details. The appearance control sits between Unload and Technical details and cycles **System → Light → Dark**. System follows live operating-system appearance changes; explicit Light and Dark override them. The preference remains in UI memory and resets to System on reload, so the public demo adds no storage or persistence.
...
- Streaming assistant text uses the same safe GitHub Flavored Markdown renderer as the completed response. A partial response hides isolated incomplete Markdown control markers and has no copy action until completion. Partial text stays in the active run only; only the accepted completed response enters conversation history. A cancelled or failed run stores a short work record (its outcome and the last completed tool steps) so the next run can continue from it; the conversation shows that run only as activity rows and never shows the record.
...
- A compact **Generated files** section after the matching assistant Markdown and before its performance row. Each safe current-run artifact accepted with the response shows a recognizable type icon, filename, type, and size. Code files the agent wrote to do the work (.py, .js, .mjs, .cjs, .sh) stay hidden, and the section does not appear when only code files exist. A failed execution does not show a card; a later successful recovery can show its retained safe bytes. The card opens a verified owner-only temporary copy in the default application, and its visible **Save As…** action uses a native dialog for a durable atomic copy. Opening and saving expose progress, cancellation or success feedback, accessible names, and concise errors. When no default application exists, Save As remains available. Restored conversations retain working cards; the public demo keeps cards visible with native actions disabled and its existing unavailable explanation.
- A top-right **Technical details** control that opens a wider right-side drawer. The drawer uses the application surface and reduces the conversation workspace instead of covering it. Its **Overview** tab contains separate live RAM and VRAM or unified-memory allocations, their total, the runtime memory budget, allocated context, hardware cap and rationale, limits, and guest capabilities. Its separate **Steps** tab contains the ordered step list, generated scripts or commands, exact committed bytes for a workspace path-only Python or Node step, the live absolute path with null source text for a direct `/source/...` step, and complete execution evidence. Generated-file metadata does not appear in Technical details.
...
- While a step is generating, a collapsed **Thinking…** row appears in its activity list. When the segment ends, the label states how long it took, such as **Thought for 5 seconds**, or changes to **Thought** when no duration is available. Selecting the row opens the current typed thought segment across the full activity width without a separate background or inset rule. New text follows the last line while the reader stays within 50 pixels of the bottom. If the reader scrolls farther away, the position does not change. The text stays in session-scoped desktop memory. The user can open it again until Garden Desk closes. In a restored application, the row stays selectable and explains that its temporary thinking text is no longer available.
- When a run must wait for its no-network microVM to start, a **Starting the secure workspace** row appears at that point in the activity list. When the start ends, the row changes to **Started the secure workspace.** and its detail states the duration. Core keeps this start in memory only, so the row is gone after Garden Desk restarts.
...
A build with one runnable generation model shows its human-readable name and current state in the conversation header. The model loads on first use, remains resident and ready between turns, and can be unloaded manually only while idle. An acknowledged Stop discards the partial response, returns the model to `ready`, and lets the next message continue the same session or start work in a different session without a model reload. An acknowledged generation timeout also keeps the model ready. If the worker does not return the cancellation result within one second, Core contains and unloads the failed worker. After an unload, the next message loads the same approved model again. Hardware detection chooses the model-plus-context budget without a settings surface. On an 8 GB Mac the header and alert explain that local inference is unsupported, and the composer cannot submit agent work. There is no picker or arbitrary configuration affordance. A future multi-model build may show only installed, signed, hardware-compatible choices.
