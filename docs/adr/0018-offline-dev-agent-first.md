...
The agent's tool set is `bash`, `python`, `node`, `read`, `glob`, `grep`, `list`, `write`, `edit`, `image`, `skill`, `task`, `question`, and `review`. `read`, `glob`, `grep`, and `list` inspect the guest; `write` and `edit` change files only under `/workspace`; `python`, `node`, and `bash` run inside the guest. `task` starts a child agent in the same session and VM. On 2026-09-11 the owner approved fixed specialists: the primary agent can select one for a separate body of work. Children run one at a time and return findings to the parent. Explicit commands select the same packaged definition and can return requested final files. Specialized instructions load on demand through the `skill` tool for an agent that has it; a fixed specialist receives the bodies of its listed skills inside its instructions. The main agent can use `review` to call the existing document review process for one guest file. It receives findings and a separate numbered text path; the review has no tools and does not change the chat title. A run stops after at most 40 model turns. Every file created or changed under `/workspace` during a run is delivered to the user as an artifact; Core's own bookkeeping paths (`.garden-desk-tools/`, `.garden-desk-output/`) are the only exclusion, and Core makes no separate internal-versus-deliverable classification. The desktop Generated files list hides agent-written code files; the run record keeps them.
...
The host-native inference worker remains separately sandboxed so local acceleration is available. It renders tool declarations, calls, and results in Gemma 4's own native tool format through a Garden Desk-owned chat wrapper, and parses the returned call from the generated tokens by token id instead of through a generic JSON grammar, so generated source keeps every byte the model wrote. The guest cannot choose a model path or connect to the inference worker directly.
...
## Owner amendment: explicit commands (2026-09-08)

The owner approved Markdown commands and a fixed `/review` workflow. Command execution is separate from the generic agent loop. `/review` performs one guest text extraction and one model call without tools, history compaction, or a session summary. The no-network microVM and read-only source boundary remain unchanged. See the Commands section in the implementation plan.
