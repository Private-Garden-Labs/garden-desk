...
- macOS development builds sign ad-hoc. A public production build runs on the owner's Mac with `APPLE_SIGNING_IDENTITY` set to the Developer ID Application identity in the login keychain; the sidecar, the inference runtime, and the VZ helper are then signed with the hardened runtime and a timestamp before Tauri signs the app. `APPLE_API_KEY`, `APPLE_API_ISSUER`, and `APPLE_API_KEY_PATH` let Tauri notarize and staple the app, and the build then notarizes and staples the DMG. The DMG step asks macOS to let the terminal control Finder for the window layout; allow it, or set `CI=true` to build the DMG without the layout.
