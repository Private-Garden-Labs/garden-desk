...
- Explicitly not doing before V1: canonical document ingestion, OCR/layout routing, hybrid retrieval, deterministic citation verification, domain-specific Core behavior, professional workflows outside the approved prompt-only review skills and fixed specialists, diagnosis, treatment, triage, autonomous approval or denial, Knowledge Bundle import, external integrations, or model downloads.
...
The main pane is conversation-first. Its header shows the conversation name with a rename control, one model control that holds the on-device residency state and an idle-only manual unload action, and a Technical details control. The approved model name stays in the tooltip of the model state and in Technical details. It shows all streamed assistant output as safe GitHub Flavored Markdown, including single-line responses; user messages remain literal text. Partial response text stays in active memory and uses a stable draft row; only the accepted completed response is stored as a shown message. A cancelled or failed run stores a short work record (its outcome and the last completed tool steps) for the next run; the conversation shows that run only as activity rows. Long response code blocks and tables use keyboard-accessible, window-bounded scroll areas. It also shows transient typed thought segments when the approved model supports them, concise code/tool activity, warnings, failures, and cancellation state in chronological order. A collapsed **Thinking…** activity row exposes its typed thought text on request, changes to **Thought for…** when a completed duration is available or **Thought** otherwise, and keeps the text only in desktop memory until the application closes. A completed row without that temporary text opens the same preview with a one-line availability explanation. An expanded activity row can show a bounded, scrollable detail preview. Explicitly requested generated-file deliverables appear beneath their matching assistant response and before response-speed metrics, with Open and Save As actions. Complete scripts, commands, intermediate files, and logs remain in Technical details Steps. Successful execution activity uses plain-language step completion rather than runtime names or exit codes. The right-side Technical details drawer reduces the conversation workspace instead of covering it; its Overview tab shows a two-column table with no borders for the session ID, catalog path, session folder `/workspace`, mount paths and storage limits, guest operating system, recorded microVM limits, microVM network access (`false`), model name and state, GPU memory allocation and budget, and context use. Keep the known GPU budget visible when its allocation is unavailable. Save each run's last reported context use and allocation so they remain available after completion and when the session is opened again. Missing measurements show “Not available”; session-specific rows show “No session selected” before a session exists. Tools and runtimes remain expandable. Fixed controls below the table create and reveal a private AI-agent debugging snapshot with SQLite-backed session records and bounded microVM logs. The snapshot contains saved workspace files and a file list; it does not open the live guest folder. Its separate Steps tab contains one ordered step list that exposes generated code, bounded live output, errors, and typed VM diagnostics one collapsed step at a time, together with the recorded prompt, requested result shape, decision, termination, and exit code read on demand for that step's run. Read-only step evidence boxes resize vertically to a window-bounded maximum, wrap text on request, and copy their exact text to the local clipboard. The preview action for an inline activity step opens that tab and step directly. Assigned Python and Node source files show their guest filename, language, and local syntax highlighting. The composer remains anchored at the bottom. Arbitrary model/runtime configuration stays out of the ordinary interface.
...
- **Tools**: `bash`, `python`, `node`, `read`, `glob`, `grep`, `list`, `write`, `edit`, `image`, `skill`, `task`, `question`, and `review`. `read`, `glob`, `grep`, and `list` inspect the guest; `read` returns numbered text for DOC, DOCX, and PDF files through the same fixed guest extraction as the review workflow, and plain UTF-8 text otherwise, caching one extracted document under `/workspace/.garden-desk-tools/` so repeated reads extract once; `write` and `edit` change files only under `/workspace`; `bash`, `python`, and `node` run inside the guest. `task` selects a packaged child agent by its name and description. The main agent can select a specialist for a separate body of work. Children run one at a time in the same session and VM. An agent's optional `skills` list names the skills it uses: an agent with the `skill` tool sees and loads only those (all skills when the list is absent); an agent without the `skill` tool receives their text inside its instructions. The main agent can call `review` for a text check of one document.
...
- **Artifacts**: every file created or changed under `/workspace` during a run is delivered to the user; Core makes no internal-versus-deliverable classification. The desktop Generated files list hides the code files the agent wrote to do the work (`.py`, `.js`, `.mjs`, `.cjs`, `.sh`); the run record keeps them.

Core mediates model completions through the host-native inference worker, using the pinned server’s native chat template and tool parser; the guest has no inference channel. Idle VMs sit in a least-recently-used pool bounded by total RAM after the inference cap and host reserve; different conversations may run concurrently up to that capacity, with model turns queued through one resident inference worker.

Fixed specialists use packaged agent Markdown with their subject and workbook skills included at load, so a specialist run has no skill selection step. The main agent chooses one path in a fixed order: `review` for a review or proofread of one document that needs no calculation, the named specialist whose description matches, `general` for other document or data work on existing files, including domain reviews that no specialist covers, `explore` for code questions, and direct work for scripts, commands, file operations, new content, or a final file or check from existing findings. The main agent's instructions list each specialist with its description; when a specialist matches, the main agent delegates before it lists or reads the source files, even for a small folder. Matter chronology builds a cited timeline across legal records. Contract obligations extracts duties and terms from agreements and amendments; `/obligations` selects that same workflow. Document comparison reports changes across supplied versions with references to each source. Financial review matches records across files and checks invoice and expense calculations, supporting evidence, and supplied rules; `/reconcile` and `/expenses` select that workflow. Delegated specialists return cited findings and working evidence under `/workspace/.garden-desk-tools/<child-run-id>/`; the main agent owns final files. Each `task` and `review` call names the work from the user's request that the main agent will still do after that turn. When every child call in a turn succeeds and names none, Core returns the child answers of that turn to the user unchanged and ends the run. Each child receives the assigned request, the user's latest request word for word, and attachment metadata, without the full conversation. Its saved identity, full assigned task, and parent tool-call ID link its activity to the parent. Parent snapshots include child summaries; completed child findings remain available after restart. Partial response text stays in memory.
...
The owner approved explicit commands on 2026-09-08. `prompts/commands/*.md` supplies names, descriptions, and instructions. Core lists them through `commands.list`; typing `/` opens the desktop command menu. Selection inserts the command without sending the message. Command discovery and execution are separate from the agent loop under `packages/core/src/commands/`.

Commands use the normal agent unless their Markdown selects a fixed workflow. `/review` extracts exactly one attached DOC, DOCX, text PDF, TXT, or MD file inside the no-network microVM. It saves numbered text under `/workspace/.garden-desk-tools/`, then sends the full extraction and the current request to one model call with no tools. It sets no output token limit; the thinking level and fitted context are the only bounds (owner decision, see [ADR 0020](adr/0020-ternary-bonsai-2-prism-fork.md)). It returns the review in chat. For a new chat, the model generates a short title in the review heading during the same request. Core saves the completed heading before the review finishes, so the title remains if the review fails. Existing chat titles stay unchanged. It does not use conversation history, compact the text, retry, create a report, or queue a session summary. Unsupported, empty, truncated, or oversized input stops the review. Extraction does not preserve all visual or embedded content. Document instructions remain untrusted data.

The main agent can call `review({ path, prompt })` for one file under `/source`, `/run/attachments`, or `/workspace`. It uses the same extraction, packaged instructions, and model limits as `/review`. Each call saves numbered text under `/workspace/.garden-desk-tools/review-<id>/` and returns its path, source path, and findings. The main agent keeps control of the final answer. Each internal review runs as a child run under its review step, with its own extraction, model call, and findings; it does not change the chat title or stream into the final answer. Children cannot call this tool. Failed reviews return a failed tool result; cancellation stops the operation.
...
A command can name an `agent` instead of repeating its workflow instructions. The specialist runs in the main run and creates no child run; the run records the selected specialist. It gets the same packaged instructions, skills, tools, and working directory as the `task` specialist, with the current command request and attachments, and without the conversation history. It cannot delegate, ask the user, or call the `review` tool. This explicit run returns findings directly and can create final files when requested. It does not require a second main-agent pass. `/review` keeps its separate fixed workflow.
...
The public release is built by hand on a physical Mac and a physical Windows machine: the macOS application bundle as a disk image and the Windows application directory as a zip archive, named `Garden-Desk-<version>-macos-arm64.dmg` and `Garden-Desk-<version>-windows-x64.zip`. Both files are uploaded directly to Cloudflare. The public site links them and lists the SHA-256 of each file. No repository workflow builds or publishes a release.

Before the packaged host launches sensitive children, it verifies the sidecar and the signed-application-anchored resource manifest, verifies fixed native helpers and every prompt asset against that manifest, and retains non-writable read locks until shutdown so a user-writable copy cannot swap children or instructions after verification. The setup helper receives the same verification and lock before elevation. The webview retains the same fixed capability set and cannot invoke arbitrary process or setup operations. The desktop selects the inference envelope automatically. A Mac needs at least 16 GiB installed unified memory: below 24 GiB the inference budget is 10 GiB, and at 24 GiB and above it is 16 GiB. On Windows, the worker selects one usable dedicated GPU first, and the inference budget follows the memory that GPU reports as usable, up to 16 GiB. It selects that GPU only when the budget holds the model, the smallest supported context, and the margin. If no dedicated GPU is usable, it selects one integrated GPU, which still needs 16 GiB usable GPU allocation and 24 GiB installed RAM. Brand and speed do not decide support. The worker uses an FP16 context cache and fits the largest context inside that budget, between 32K and 128K tokens. It reports the actual allocation, memory kind, backend, and one selected device. Stop and generation timeouts do not unload a healthy model. Manual unload, a model or operation change, image inspection, or Core shutdown terminates the complete worker process.
...
- Keep the generic `read` tool as strict streamed UTF-8 plain-text inspection for plain files, and return numbered DOC, DOCX, and PDF text from the fixed guest extraction. Invalid UTF-8 and NUL bytes fail. Clamp safe optional integers to their configured bounds, and reject wrong types, non-finite numbers, and unsafe integers. Limit every model-facing tool-result preview to 50 KiB after JSON encoding; spill a complete oversized result under `/workspace` and name its path in the tool result. Add generic `write` and `edit` tools scoped to `/workspace`, with `edit` replacing the first match of the old text and ignoring differences in whitespace, line endings, and typographic dashes and quotes.
- Treat every file created or changed under `/workspace` during a run as an artifact delivered to the user; do not parse task text or add a suffix or format allowlist in Core. Hide agent-written code files from the desktop Generated files list only.
