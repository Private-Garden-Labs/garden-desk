...
Fixed specialists use packaged agent Markdown with their subject and workbook skills included at load, so a specialist run has no skill selection step. The main agent chooses one path in a fixed order: `review` for one document, the named specialist whose description matches, `general` for other document or data work on existing files, including domain reviews that no specialist covers, `explore` for code questions, and direct work for scripts, commands, file operations, new content, or a final file or check from existing findings. The main agent's instructions list each specialist with its description; when a specialist matches, the main agent delegates before it lists or reads the source files, even for a small folder. Folder intake identifies current file groups and structures before batch work. Matter chronology builds a cited timeline across legal records. Contract obligations extracts duties and terms from agreements and amendments; `/obligations` selects that same workflow. Document comparison reports changes across supplied versions with references to each source. Financial record extraction creates source-linked records from supplied files; `/extract` selects that workflow. Financial reconciliation matches records and reports differences; `/reconcile` selects that workflow. Invoice and expense review checks calculations, supporting evidence, and supplied rules; `/expenses` selects that workflow. Evidence brief assembles a short cited answer with conflicts and open questions; `/brief` selects that workflow. Delegated specialists return cited findings and working evidence under `/workspace/.garden-desk-tools/<child-run-id>/`; the main agent owns final files. Each child receives the assigned request and attachment metadata, without the full conversation. Its saved identity, full assigned task, and parent tool-call ID link its activity to the parent. Parent snapshots include child summaries; completed child findings remain available after restart. Partial response text stays in memory.
