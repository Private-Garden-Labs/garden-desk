...
def is_artifact_candidate(path):
    return not (
        path == ".garden-desk-tools"
        or path.startswith(".garden-desk-tools/")
        or path == ".garden-desk-output"
        or path.startswith(".garden-desk-output/")
    )
...
def execute(connection, request, previous):
    ...
    environment = {
        ...
        "GARDEN_DESK_WORKSPACE_DIR": str(WORKSPACE),
    }
