...
it("replaces the development runtime without overwriting a loaded executable", async () => {
  ...
  const relative = join("resources", "core", "inference", "macos-arm64");
  const source = join(target.tauriRoot, relative);
  const destination = join(target.tauriRoot, "target", "debug", relative);
  await mkdir(source, { recursive: true });
  await mkdir(destination, { recursive: true });
  await writeFile(join(source, "llama-server"), "new runtime");
  await writeFile(join(destination, "llama-server"), "loaded runtime");
  const loaded = await open(join(destination, "llama-server"), "r");
  try {
    await prepareDevelopmentRuntimeOutput(target.desktopRoot);
    expect(await readFile(join(destination, "llama-server"), "utf8")).toBe("new runtime");
