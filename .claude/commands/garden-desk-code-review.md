...
3. Choose the mode. With a reviewed sha, run `gh api repos/<owner>/<repo>/compare/<reviewed-sha>...<head-sha> --jq '{status, files: [.files[] | {filename, additions, deletions, patch}]}'`. If `status` is `ahead` and `files` is not empty, the mode is incremental and that output is the diff to review; also collect the earlier claude inline findings with `gh api repos/<owner>/<repo>/pulls/<number>/comments --jq '.[] | select(.user.login == "claude[bot]") | {path, line, body}'`. If `status` is `identical`, post nothing and stop. Otherwise (no reviewed sha, `behind`, `diverged`, or a failed command) the mode is full.
   ...
   For the single-agent tier, skip steps 4 through 6. Launch one `review-opus` agent with timeout `300000` and a 290-second budget. Give it the complete `REVIEW.md`, the paths of `CLAUDE.md` and its imports, the mode, the exact diff command, earlier findings in incremental mode, and all four lenses. It reads the diff once, creates a short change summary, reviews every lens, validates each finding against the code, and discards every finding that it cannot confirm. It returns the summary and confirmed issues in the step 5 shape. If it fails, review the diff yourself with the same rules, add `Lenses not completed: all (single agent failed)`, and report no findings. Wait one more time for 60 seconds only when the first wait times out. Then continue at step 7.

   For the full-suite tier, continue at step 4.
...
7. Output the validated findings to the terminal using the "Summary Format" from `REVIEW.md`, including the status, recommendation, and tier. If `--comment` was NOT provided, stop here.
...
9. If `--comment` was provided, post one `gh pr comment` with the "Summary Format": the status and recommendation line, tier line, highest-risk sentence or "No blocking issues.", any "Lenses not completed" line, and the closing `Reviewed <head-sha>` line. Post it in both modes, even with zero findings, so the next run knows where to continue.
